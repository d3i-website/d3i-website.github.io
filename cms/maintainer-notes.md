...
- **`component`** — a named slot rendered after the section: `institutions-grid` (Networks), `script-selector` (the embed). Add new components in the `case` block of the include. The key is `widget: hidden` in the CMS so editors can't change it but saves preserve it.
- **`routes`** — optional list rendered after the figures by `_includes/routes.html`: a fork the reader resolves by picking the question that fits them. Each route is `question` / `answer` / `steps` (each step: `text` plus a `link` of `label` + `url`), drawn as a coloured rail (blue, then yellow, alternating) with a stop per step; two routes sit side by side above 900px and stack below. A developer-owned `glyph` key (`netherlands` | `globe`, hidden in the CMS) draws an SVG from `_includes/glyphs/` at the head of the rail. The Netherlands outline is traced from Eurostat GISCO Countries 2024 (1:20M, © EuroGeographics for the administrative boundaries) and simplified to ~40 points; the include prints the required EuroGeographics credit line under the routes whenever that glyph is used, so editors never have to. Used on Getting started.
- **Next step block** — not a section key: `_includes/journey-next.html`, hooked into the `page` and `page-sections` layouts above the contact block, reads the section's hub-journey page (matched by `hub_section` = the page's `sidebar.nav`) and renders the step after the one whose link matches the current URL, plus an "After that" line for the step beyond. Driven entirely by the hub's Journey steps in the CMS; nothing to configure per page. Renders nothing on the last step, on the hub itself, or on pages that aren't steps (e.g. Port in education).
- **Untitled sections** — a section with no `title`, `body`, `figures`, or `routes` renders only its component, which is how the Script selector page drops the embed between two prose blocks. Its anchor id falls back to the component name.
...
- **Script selector settings** (`_data/script_selector.yml`): the embed URL (with `?embed=1` so the app can detect framing), availability toggle, and fallback copy. The URL changes when the SURF workspace moves; keep it developer-owned.
