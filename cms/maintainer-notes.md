...
- **`component`** — a named slot rendered after the section: `institutions-grid` (Networks), `script-builder` (the embed). Add new components in the `case` block of the include. The key is `widget: hidden` in the CMS so editors can't change it but saves preserve it.
...
- **Untitled sections** — a section with no `title`, `body`, `figures`, or `routes` renders only its component, which is how the Script builder page drops the embed between two prose blocks. Its anchor id falls back to the component name.
